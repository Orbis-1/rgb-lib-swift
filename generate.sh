...
rustup target add aarch64-apple-ios x86_64-apple-ios --toolchain $TOOLCHAIN
rustup target add aarch64-apple-ios-sim --toolchain $TOOLCHAIN
rustup target add aarch64-apple-darwin x86_64-apple-darwin --toolchain $TOOLCHAIN

echo "Generating Swift bindings..."
cargo +$TOOLCHAIN run "${MANIFEST_PATH[@]}" \
...
for target in "${TARGET_TRIPLES[@]}"; do
  ...
  if [ "$target" == "aarch64-apple-ios" ]; then
    IPHONEOS_DEPLOYMENT_TARGET=16.0 CFLAGS="-mios-version-min=16.0" \
      ...
      cargo +$TOOLCHAIN build "${MANIFEST_PATH[@]}" --target "$target"
  elif [ "$target" == "aarch64-apple-ios-sim" ]; then
    # special build for M1 ios simulator (requires -Z build-std)
    cargo +$TOOLCHAIN build -Z build-std \
